...
int create_packets(char *eth_dest, char *ip_dest, uint8_t ip_protocol, uint8_t payload, unsigned int payload_size)
{
    /* Getting MAC and IP address for source */
    char *eth_host = get_mac_address("eth0"); /* Change name of interface depending on fit */
    char *ip_host = get_ip_address("eth0"); /* Change name of interface depending on fit */
    ...
    if (ip_protocol == ip_protocol_tcp)
    {
        ...
        packet_size = (uint8_t)(payload_size + sizeof(tcp_hdr_t) + sizeof(gre_hdr_t) + sizeof(ip_hdr_t) + sizeof(ethernet_hdr_t));        
    /* Checking if the ne packet requires ICMP header */
    } else if (ip_protocol == ip_protocol_icmp)
    {
        /* Setting the new packet size */
        packet_size = (uint8_t)(payload_size + sizeof(icmp_hdr_t) + sizeof(gre_hdr_t) + sizeof(ip_hdr_t) + sizeof(ethernet_hdr_t));
    ...
    ip_hdr_t *new_ip = (ip_hdr_t *)(new_packet + sizeof(ethernet_hdr_t) + sizeof(gre_hdr_t));
    new_ip->ip_v = 4;
    ...
    if (ip_protocol == ip_protocol_tcp)
    {
        tcp_hdr_t new_tcp = (tcp_hdr_t *)(new_packet + sizeof(ethernet_hdr_t) + sizeof(gre_hdr_t) + sizeof(ip_hdr_t));
    ...
    {
        icmp_hdr_t new_icmp = (icmp_hdr_t *)(new_packet + sizeof(ethernet_hdr_t) + sizeof(gre_hdr_t) + sizeof(ip_hdr_t));
    }
}